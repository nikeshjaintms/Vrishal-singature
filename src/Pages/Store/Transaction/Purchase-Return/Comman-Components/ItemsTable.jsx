...
const ItemsTable = ({ data, setData, prItems }) => {
    ...
    useEffect(() => {
        if (prItems?.items?.length > 0) {
            const initializedItems = prItems?.items?.map(item => ({
                ...
                discount_amount: 0,
                sp_discount: item?.sp_discount || 0,
        ...
        setData(prevState =>
            prevState.map((item, i) => {
                if (i === index) {
                    ...
                    if (name === "quantity") {
                        if (!value) {
                            updatedItem.errors.pur_quantity = "Required";
                        } else if (value > item?.pu_quantity) {
                            updatedItem.errors.pur_quantity = "PUR quantity is not greaterthan pu quantity  " + item.pu_quantity
                        } else {
                            updatedItem.errors.pur_quantity = ""
                        }
                    }
                                ...
                                <tbody>
                                    {commentsData.length > 0 ? (
                                        commentsData.map((row, rowIndex) => (
                                            <tr key={rowIndex}>
                                                <td>{rowIndex + 1}</td>
                                                <td>{row?.item_data?.name}</td>
                                                <td>{row.unit}</td>
                                                <td>{row.mcode}</td>
                                                <td>{row.pu_quantity}</td>
                                                <td >
                                                    <input type="number" placeholder="Enter value" className='form-control' style={{ width: '100px' }} onChange={(e) => handleChange(e, rowIndex)} name='quantity' value={row?.quantity} />
                                                    {row?.errors?.pur_quantity && (
                                                        <div className="error">{row.errors.pur_quantity}</div>
                                                    )}
                                                </td>
                                                <td>{row.rate}</td>
                                                <td>{row.amount}</td>
