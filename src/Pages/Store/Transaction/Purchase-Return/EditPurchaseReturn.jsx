...
import EditItemsModel from '../Purchase-Recieving/Comman-Components/EditItemsModel';
...
const EditPurchaseReturn = () => {
    ...
    const handleSave = (itemData, i_id) => {
        if (modalMode === 'add') {
            ...
            const myurl = `${V_URL}/user/add-pur-item`;
            axios({
                method: "POST",
                url: myurl,
                data: payload,
            ...
            const myurl = `${V_URL}/user/update-pur-item`;
            axios({
                method: "PUT",
                url: myurl,
                data: payload,
    ...
    useEffect(() => {
        ...
        if (disable === true) {
            setEntity([]);
            getPurchaseReturnItems({ id: data?._id });
        }
    ...
    const handleDelete = (id, title, itemId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const myurl = `${V_URL}/user/delete-pur-item`;
                axios({
                    method: "PUT",
                    url: myurl,
    ...
    const handleSubmit = () => {
        const selectedTransport = transport?.find((item) => item?._id === formData?.transport_id);
        if (formValidation()) {
            const myurl = `${V_URL}/user/update-pur`;
            const payload = {
                "id": data?._id,
