...
//                 //     }
//                 // });
// const { spool_no } = targetItem?.jointDetails?.[0] || {};

// updatedTableData.forEach((item, i) => {
//     const itemSpool = item?.jointDetails?.[0]?.spool_no;

//     if (itemSpool === spool_no) {
//         item.is_accepted = isAccepted;
//         updatedStatus[i] = isAccepted;
//     }
// });
...
const handleAcceptRejectClick = (index, isAccepted, name) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            const targetJoint = targetItem?.jointDetails?.[0];

            const targetSpool = targetJoint?.spool_no;
            const targetDrawing = targetJoint?.drawing_no;

            updatedTableData.forEach((item, i) => {
                const joint = item?.jointDetails?.[0];

                if (
                    joint?.spool_no === targetSpool &&
                    joint?.drawing_no === targetDrawing
                ) {
                    item.is_accepted = isAccepted;
                    updatedStatus[i] = isAccepted;
                }
            ...
            toast.success(
                `${name} (Spool: ${targetSpool}) ${isAccepted ? "accepted" : "rejected"}`
            );
        }
    });
};
  ...
  updatedData.forEach(item => {
    if (item.is_accepted === '' || item.is_accepted === undefined) {
      isValid = false;
      toast.error("Please accept or reject all items");
    }
  });
  ...
  if (!isValid) return;
    ...
    qc_remarks: item.qc_remarks || ''
        ...
        setDisable(true);
        const myurl = `${V_URL}/user/verify-weld-visual-inspection-piping`;
        ...
        axios({
            ...
            if (response.data?.success === true) {
                ...
                navigate('/piping/user/weld-visual-clearance-management');
            } else {
                ...
                <div className="page-wrapper">
                    <div className="content">
                        <PageHeader breadcrumbs={[
                            ...
                            { name: "Weld Visual Clearance List", link: "/piping/user/weld-visual-clearance-management", active: false },
    ...
    const joint = elem?.jointDetails?.[0];
    ...
    return (
        <tr key={elem?._id || i}>
            ...
            <td>{joint?.weldingProcess || '-'}</td>
                                                            ...
                                                            {/* <td>
                                                                <select className='form-control form-select table-select' name='wps_no'>
                                                                    <option value="">Welder No.</option>
                                                                    <option></option>                                                               
                                                                </select>
                                                            </td> */}
                                                            {editRowIndex === i ? (
                                                                <td className=''>
                                                                    <div className='d-flex gap-2'>
                                                                        <span
                                                                            ...
                                                                            onClick={() => handleAcceptRejectClick(i, true, "Item")}>
                                                                            <Check />
                                                                        </span>
                                                                        <span
                                                                            className={`absent-table attent-status ${acceptRejectStatus[i] === false ? 'selected' : ''}`}
                                                                            style={{ cursor: 'pointer' }}
                                                                            onClick={() => handleAcceptRejectClick(i, false, "Item")}
                                                                        >
                                                                            <X />
                                                                        </span>
                                                                    </div>
                        ...
                        <SubmitButton disable={disable} handleSubmit={handleSubmit}
                            link={'/piping/user/weld-visual-clearance-management'} buttonName={'Generate Weld Visual Acceptance'} />
                    </div>
                </div>
            </div>
        </>
