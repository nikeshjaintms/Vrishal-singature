...
const MioTable = ({ setSubmitArr, paintNo, data, dispatch_site, report_no }) => {
    ...
    const getDisaptchOfferTable = () => {
        ...
        dispatch(getStockSurfaseDataInMio({ DATA: payload }));
    }
...
console.log("getPipingMultiMioData======>",getPipingMultiMioData);
  useEffect(() => {
    if (!getPipingMultiMioData) return;

    setTableData(getPipingMultiMioData);
    setSubmitArr(getPipingMultiMioData);
...
console.log("commentsData=====>",commentsData);
    ...
    const handleRemoveByDrawing = async (elem) => {
        ...
        try {
            const myurl = `${V_URL}/user/piping-delete-multi-stock-mio-offer`;
 ...
 setTableData(prev => prev.filter(item => item._id !== elem._id));
    setSubmitArr(prev => prev.filter(item => item._id !== elem._id));
                // ✅ REFRESH TABLE
                dispatch(getPipingMultiStockMio());
