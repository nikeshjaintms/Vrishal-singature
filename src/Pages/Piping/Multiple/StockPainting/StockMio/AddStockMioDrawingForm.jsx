...
const AddMioDrawingForm = ({ modalOpen, handleModalClose ,paintNo, handleAddMore, handleCloseModal, editeMode, onSaveItem, updateItem, editItem, modalMode }) => {
    ...
    useEffect(() => {
        ...
        if (modalMode === "edit" && updateItem) {
            setItem({
                ...
                mio_used_grid_qty: updateItem?.mio_used_grid_qty || 0,
        ...
        // When initially entering 'editMode' with an existing editItem
            ...
            setItem({
                ...
                mio_used_grid_qty: editItem?.mio_used_grid_qty || 0,
    ...
    const validationModal = () => {
        ...
        if (!item.mio_used_grid_qty || isNaN(item.mio_used_grid_qty) || parseInt(item.mio_used_grid_qty) <= 0) {
            isValid = false;
            newErrors['mio_used_grid_qty_error'] = "Please enter a valid Qty";
        }
...
const handleSaveSingleItemToAPI = async () => {
  ...
  const formattedItemForAPI = [{
    ...
    mio_used_grid_qty: Number(item.mio_used_grid_qty),
  ...
  try {
    await dispatch(manageMioOffer({ bodyFormData })).unwrap();
    if (paintNo) await dispatch(getMultiMio({ paint_system_id: paintNo })).unwrap();
                                        ...
                                        <div className="col-md-6">
                                            <div className="input-block local-forms">
                                                ...
                                                <input
                                                    type="number"
                                                    className="form-control"
                                                    name="mio_used_grid_qty"
                                                    value={item.mio_used_grid_qty}
                                                    onChange={handleChange}
                                                />
                                                <div className="error">{err.mio_used_grid_qty_error}</div>
                                            </div>
                                        </div>
                                    </div>
...
export default AddMioDrawingForm;