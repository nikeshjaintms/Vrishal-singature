...
import { getPipingMultiStockMioClearance } from '../../../../../Store/Piping/MultiStockMIO/GetMultiStockMioClearance';
// import { getMultiMioViewPage } from '../../../../../Store/MutipleDrawing/MultiMIO/GetMultiMioViewPage';
import { getPipingMultiStockMioViewPage } from '../../../../../Store/Piping/MultiStockMIO/GetMultiStockMioViewPage';
...
const MultiMioClearance = () => {
  ...
  useEffect(() => {
    if (disable) {
      ...
      dispatch(getPipingMultiStockMioViewPage({ paint_system_id: "" }));
  ...
  useEffect(() => {
    if (disable1) {
      ...
      dispatch(getPipingMultiStockMioClearance({ paint_system_id: "", page: currentPage1, limit: limit1, search: search1 }));
      setDisable1(false);
    }
  ...
  const entity = useSelector((state) => state.getPipingMultiStockMioViewPage?.user?.data?.data);

  console.log("entity", entity);
  // const pagination = useSelector((state) => state.getMultiMioIns?.user?.data?.pagination);
  const entity1 = useSelector((state) => state.getPipingMultiStockMioClearance?.user?.data?.data);
  console.log("entity1", entity1);
  const pagination1 = useSelector((state) => state.getPipingMultiStockMioClearance?.user?.data?.pagination);
  ...
  const filterData = (data, statusFilter, searchQuery) => {
    ...
    if (searchQuery) {
      filteredData = filteredData.filter((item) =>
        ...
        [item?.procedure_no, item?.paint_system_no, item?.qc_name, item?.report_no, item?.piping_class_name]
          .some(val => val?.toLowerCase()?.includes(searchQuery?.toLowerCase()))
      );
    }
    return filteredData;
  ...
  const handleDownloadIns = (elem) => {
    ...
    PdfDownloadErp({ apiMethod: 'post', url: 'piping-download-multi-stock-mio', body: bodyFormData });
  }
        ...
        <div className="page-wrapper">
          <div className="content">
            <PageHeader breadcrumbs={[
              ...
              { name: "MIO Clearance List", link: "/piping/user/project-store/mio-clearance-management", active: false },
            ]} />
                          ...
                          <tbody>
                            {commentsData?.map((elem, i) =>
                              <tr>
                                ...
                                <td>{elem?.items?.map((item, index) => <div key={index}>{item?.item_name || "-"}</div>)}</td>
                                  ...
                                  <td>
                                    {elem?.status === 1 ? (
                                      <span style={{ cursor: 'pointer' }} onClick={() => {
                                        console.log("PIPING_VERSION - Navigating with elem:", elem);
                                        navigate('/piping/user/manage-stock-mio-clearance', { state: elem });
                                      }}>
                                        <BadgeCheck />
                                      </span>
                                    ) : <X />}
                          ...
                          <tbody>
                            {commentsData1?.map((elem, i) =>
                              <tr key={elem?._id}>
                                ...
                                <td>{elem?.items?.map((item, index) => <div key={index}>{item?.item_name || item?.spool_no || "-"}</div>)}</td>
                                ...
                                <td className="text-end">
                                  <div className="dropdown dropdown-action">
                                    ...
                                    <div className="dropdown-menu dropdown-menu-end">
                                      {/* <button type='button' className="dropdown-item" onClick={() => navigate('/piping/user/view-mio-clearance', { state: elem })}>
