...
const FinalCoatTable = ({ setSubmitArr, paintNo, data, dispatch_site, report_no, onRefresh }) => {
...
//         setTableData(getMultiStockFinalCoatData);
//         setSubmitArr(getMultiStockFinalCoatData);
//     } 
...
//         setSubmitArr([]);
//     }
// }, [getMultiStockFinalCoatData, data]);
        ...
        if (search) {
            if (search) {
                computedComments = computedComments.filter(
                    (dr) =>
                        dr?.final_coat_no?.toString()?.toLowerCase()?.includes(search) ||
                        dr?.drawing_no?.toString()?.toLowerCase()?.includes(search) ||
                        dr?.item_name?.toString()?.toLowerCase()?.includes(search) ||
                        dr?.piping_class_name?.toString()?.toLowerCase()?.includes(search)
                );
            }
        }
...
console.log("commentsData====>",commentsData);
    ...
    const handleSaveClick = async () => {
        ...
        try {
            const myurl = `${V_URL}/user/update-multi-stock-mio-offer`;
    ...
    const handleRemoveByDrawing = async (elem) => {
        ...
        try {
            const myurl = `${V_URL}/user/piping-delete-multi-stock-final-coat-offer`;
