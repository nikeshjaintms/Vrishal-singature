...
import { getPipingStockFinalCoatIns } from '../../../../../Store/Piping/MultiStockFinalCoat/GetStockFinalCoatOffer';
...
const MultiFinalCoatClearance = () => {
  ...
  useEffect(() => {
    if (disable === true || disable1 === true) {
      dispatch(getPipingStockFinalCoatIns());
      setDisable(false);
      setDisable1(false);
    }
  }, [disable, disable1]);

  const entity = useSelector((state) => state.getPipingStockFinalCoatIns?.user?.data);
  ...
  const handleDownloadIns = (elem) => {
    ...
    PdfDownloadErp({ apiMethod: 'post', url: 'piping-download-multi-stock-final_coat', body: bodyFormData });
  }
                            ...
                            {commentsData?.map((elem, i) =>
                              <tr key={i}>
                                ...
                                {ERP_ROLE === QC && (
                                  <td>
                                    {elem?.status === 1 ? (
                                      <span style={{ cursor: 'pointer' }} onClick={() => navigate('/piping/user/manage-stock-final-coat-clearance', { state: elem })}>
                                        <BadgeCheck />
                                      </span>
                                    ) : <X />}
                                  </td>
