...
const SurfaceTable = ({
  ...
  const handleRemoveByDrawing = async (elem) => {
    ...
    try {
      const response = await axios.post(
        `${V_URL}/user/piping-delete-multi-surface-offer`,
        removeItem,
        {
          headers: {
      ...
      if (response.data?.success) {
        ...
        try {
          await dispatch(
              getPipingMultiSurface({ report_no: elem.report_no || report_no, main_id: elem.main_id })
          ).unwrap();
        } catch (err) {
        ...
        toast.error(response.data?.message || "Something went wrong while removing item");
      }
    } catch (error) {
