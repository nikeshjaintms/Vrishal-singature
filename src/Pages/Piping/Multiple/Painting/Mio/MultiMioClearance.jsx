...
import { getPipingMultiMioClearance } from '../../../../../Store/Piping/MultiMIO/GetMultiMioClearance';
...
const MultiMioClearance = () => {
  ...
  useEffect(() => {
    if (disable) {
      ...
      dispatch(getPipingMultiMioViewPage({ paint_system_id: "" }));
  ...
  useEffect(() => {
    if (disable1) {
      ...
      dispatch(getPipingMultiMioClearance({ paint_system_id: "", page: currentPage1, limit: limit1, search: search1 }));
      setDisable1(false);
    }
  ...
  const entity1 = useSelector((state) => state.getPipingMultiMioClearance?.user?.data?.data);
  console.log("entity1", entity1);
  const pagination1 = useSelector((state) => state.getPipingMultiMioClearance?.user?.data?.pagination);
                          ...
                          <tbody>
                            {commentsData?.map((elem, i) =>
                              <tr>
                                ...
                                <td>{elem?.items?.map((item, index) => <div key={index}>{item?.drawing_no || "-"}</div>)}</td>
                                <td>{elem?.items?.map((item, index) => <div key={index}>{item?.rev || "-"}</div>)}</td>
                                  ...
                                  <td>
                                    {elem?.status === 1 ? (
                                      <span style={{ cursor: 'pointer' }} onClick={() => {
                                        ...
                                        navigate('/piping/user/manage-mio-clearance', { state: elem });
                                      }}>
                                        <BadgeCheck />
                                      </span>
                                    ) : <X />}
                          ...
                          <tbody>
                            {commentsData1?.map((elem, i) =>
                              <tr key={elem?._id}>
                                ...
                                {/* <td>{elem?.report_no}</td> */}
                                <td>{elem?.items?.map((item, index) => {
                                  const drawing = elem?.drawingDocs?.find(d => d._id === item?.drawing_id);
                                  return (
                                    <div key={index}>
                                      {item?.drawing_no || drawing?.drawing_no || "-"}
                                    </div>
                                  )
                                })}</td>
                                <td>{elem?.items?.map((item, index) => <div key={index}>{item?.rev || "-"}</div>)}</td>
