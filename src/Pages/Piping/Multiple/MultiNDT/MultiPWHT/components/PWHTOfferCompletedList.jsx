...
useEffect(() => {
  setCurrentPage(1);
    ...
    const entity = useSelector((state) => state.getPwhtInspectionPiping?.dataByStatus["ALL"]?.data);
    console.log("entity",entity);
    const pagination = useSelector((state) =>state.getPwhtInspectionPiping?.dataByStatus["ALL"]?.pagination);
    ...
    const handleDownloadOffer = (elem) => {
        ...
        PdfDownloadErp({ apiMethod: 'post', url: 'download-multi-pwht-offer-pdf-piping', body: bodyFormData });
    }
                                ...
                                <div className="row align-items-center">
                                    <div className="col">
                                        <div className="doctor-table-blk">
                                            <h3>Completed PWHT Offer List</h3>
                                            ...
                                            <tbody>
                                                {commentsData?.map((elem, i) =>
                                                    <tr key={elem?._id}>
                                                        ...
                                                        <td>{[...new Set(elem?.items?.map(i => safeRender(i?.drawing_no)).filter(Boolean))].join(", ")}</td>
                                                        <td>{[...new Set(
  elem?.items?.map(i => safeRender(i?.spool_no)).filter(Boolean)
)].join(", ")}
</td>
                                                        <td>{[...new Set(
  elem?.items?.map(i => safeRender(i?.joint_no)).filter(Boolean)
)].join(", ")}
</td>
<td>{[...new Set(
  elem?.items?.map(
    i => safeRender(i?.piping_material_specification)
  ).filter(Boolean)
)].join(", ")}
</td>
                                                       
                                                       <td>{safeRender(elem?.offered_by_name)}</td>
...
export default PWHTOfferCompletedList