...
const offeringData = useSelector(
  (state) => state.getPwhtInspectionPiping.dataByStatus["1"]?.data
);

const offeringPagination = useSelector(
  (state) => state.getPwhtInspectionPiping.dataByStatus["1"]?.pagination
);

const clearanceData = useSelector(
  (state) => state.getPwhtInspectionPiping.dataByStatus["2,3,4"]?.data
);

const clearancePagination = useSelector(
  (state) => state.getPwhtInspectionPiping.dataByStatus["2,3,4"]?.pagination
);
...
const handleRefresh1 = () => {
    ...
    const handleDownload = (elem) => {
        ...
        PdfDownloadErp({ apiMethod: 'post', url: 'download-multi-pwht-inspection-pdf-piping', body: bodyFormData });
    }
    return (
        <>
                ...
                <div className="page-wrapper">
                    <div className="content">
                        ...
                        <PageHeader breadcrumbs={[
                            ...
                            { name: " PWHT List", link: "/piping/user/pwht-offer-management", active: false },
                            { name: `${data?._id ? 'Edit' : 'Add'}  PWHT Offer`, active: true }
                        ]} />
                                                    ...
                                                    <tbody>
                                                        {offeringData?.map((elem, i) =>
                                                            <tr key={elem?._id}>
                                                                ...
                                                                <td>{elem?.offered_date ? moment(elem?.offered_date).format('YYYY-MM-DD') : '-'}</td>
                                                                {localStorage.getItem('ERP_ROLE') === QC && (
                                                                    <td>
                                                                        {elem?.status === 1 ? (
                                                                            <span style={{ cursor: 'pointer' }} onClick={() => navigate('/piping/user/manage-pwht-clearance', { state: elem })}>
                                                                                <BadgeCheck />
                                                                            </span>
                                                                        ) : <X />}
                                                                    </td>
                                                                ...
                                                                <td className="text-end">
                                                                    <div className="dropdown dropdown-action">
                                                                        ...
                                                                        <div className="dropdown-menu dropdown-menu-end">
                                                                            <button type='button' className="dropdown-item" onClick={() => navigate('/piping/user/manage-pwht-clearance', { state: elem })}><i
