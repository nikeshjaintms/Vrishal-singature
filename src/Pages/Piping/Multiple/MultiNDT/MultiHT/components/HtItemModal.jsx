...
const LptItemModal = ({ title, tableObj, modalOpen, handleCloseModal }) => {

    // console.log(tableObj, '2@@@')
    ...
    const [lptItems, setLptItems] = useState([]);
    const [editRowIndex, setEditRowIndex] = useState(null);
    const [lptTableData, setLptTableData] = useState([]);
    ...
    const fetchLptTable = () => {
        ...
        axios({
            ...
            if (response?.data?.success === true) {
                setLptTableData(response?.data?.data?.items)
            }
        }).catch((error) => {
            console.log(error);
        });
    ...
    useEffect(() => {
        if (tableObj?._id) {
            ...
            const updatedTableData = tableObj?.items?.map((item) => {
                ...
                const filterRTTable = lptTableData?.filter((rt) => rt?.ndt_type_id === tableObj?.ndt_type_id?._id && rt?.ndt_master_id === tableObj?.ndt_master_id?._id);

                const matchingRTItems = filterRTTable?.filter(
                    (lpt) =>
                        lpt?.grid_item_id?._id === item?.grid_item_id?._id &&
                        lpt?.grid_item_id?.drawing_id?._id === item?.drawing_id &&
                        lpt?.deleted === false &&
                        lpt?.offer_status !== 5 && lpt?.offer_status !== 4
                ) || [];

                const totalMatchingMPTUseQty = matchingRTItems?.reduce((sum, lpt) => sum + (lpt?.lpt_use_qty || 0), 0);
            ...
            setLptItems(updatedTableData);
        }
    }, [tableObj?._id, stockData, lptTableData, modalOpen]);

    const commentsData = useMemo(() => {
        let computedComments = lptItems;
        if (search) {
            computedComments = computedComments.filter(
                (lpt) =>
                    lpt.grid_item_id?.drawing_id?.drawing_no?.toLowerCase()?.includes(search?.toLowerCase()) ||
                    lpt.grid_item_id?.drawing_id?.assembly_no?.toLowerCase()?.includes(search?.toLowerCase()) ||
                    lpt.grid_item_id?.grid_id?.grid_no?.toLowerCase()?.includes(search?.toLowerCase())
            );
        }
    ...
    }, [currentPage, search, limit, lptItems]);
    ...
    const handleEditFormChange = (e) => {
        ...
        if (name === 'lpt_use_qty') {
            if (value === "") {
                ...
                const maxQty = lptItems[editRowIndex]?.offer_used_grid_qty || 0;
    ...
    const handleSaveClick = () => {
        const updatedData = lptItems.map((item) => {
            if (item.grid_item_id?.grid_id?.grid_no === lptItems[editRowIndex]?.grid_item_id?.grid_id?.grid_no) {
                return { ...item, ...editFormData, lpt_use_qty: editFormData.lpt_use_qty || "" };
            }
            return item;
        });
        setLptItems(updatedData);
    ...
    const handleSave = () => {
        const updatedData = lptItems.filter((it) => it?.lpt_use_qty > 0);
        ...
        const finalArray = updatedData?.map((it) => ({
            ...
            lpt_use_qty: it?.lpt_use_qty,
            lpt_balance_qty: parseInt(it?.offer_used_grid_qty) - parseInt(it?.lpt_use_qty),
        ...
        let storedMasterIds = JSON.parse(localStorage.getItem('LPT_TYPE_MASTER_IDS')) || [];
        ...
        localStorage.setItem('LPT_TYPE_ID', tableObj?.ndt_type_id?._id);
        localStorage.setItem('LPT_TYPE_MASTER_IDS', JSON.stringify(storedMasterIds));
