...
useEffect(() => {
  setCurrentPage(1);
    ...
    const entity = useSelector((state) => state.getLptNdtInspectionPiping?.dataByStatus["ALL"]?.data);
    console.log("entity",entity);
    const pagination = useSelector((state) =>state.getLptNdtInspectionPiping?.dataByStatus["ALL"]?.pagination);
    ...
    const handleDownloadOffer = (elem) => {
        ...
        PdfDownloadErp({ apiMethod: 'post', url: 'download-multi-lpt-ndt-offer-pdf-piping', body: bodyFormData });
    }
  ...
  elem?.items?.map(i => i?.spool_no).filter(Boolean)
                                                        ...
                                                        <td>{[...new Set(
  elem?.items?.map(i => i?.joint_no).filter(Boolean)
)].join(", ")}
</td>
<td>{[...new Set(
  elem?.items?.map(
    i => i?.piping_material_specification
  ).filter(Boolean)
)].join(", ")}
</td>
