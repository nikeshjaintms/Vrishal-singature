...
import { getMultiStockPackingSummaryPiping } from "../../../../Store/Piping/MultiStockPacking/GetMultiStockPackingSummaryPiping";
...
const MultiPackingSummary = () => {
  ...
  const data = useSelector(
    (state) => state?.getMultiStockPackingSummaryPiping?.user?.data?.data || []
  );

  const pagination = useSelector(
    (state) => state?.getMultiStockPackingSummaryPiping?.user?.data?.pagination || {}
  );
  ...
  useEffect(() => {
    dispatch(
      getMultiStockPackingSummaryPiping({
        page: currentPage,
        limit,
        search,
      })
  ...
  useEffect(() => {
   ...
   const handlePdfDownload = () => {
      ...
      PdfDownloadErp({ apiMethod: 'post', url: 'piping/export-multi-stock-packing-summary-xlsx', body: bodyFormData });
  }
            ...
            <div className="row">
              <div className="col-sm-12">
                <ul className="breadcrumb">
                  ...
                  <li className="breadcrumb-item active">Stock Packing List Summary</li>
                </ul>
              </div>
            </div>
          </div>
                    ...
                    <div className="row align-items-center">
                      <div className="col">
                        <div className="doctor-table-blk">
                          <h3>Stock Packing List Summary</h3>
                          ...
                          processedRows.map((elem, i) => {
                            ...
                            return items.map((item, index) => (
                              <tr key={`${elem._id}-${index}`}>
                                ...
                                <td>{item?.total_packaged_qty || "-"}</td>
                              
                                <td>
                                  {Array.isArray(item?.merged_imir_no) && item.merged_imir_no.length > 0
                                    ? item.merged_imir_no.map((imir, idx) => (
                                        <React.Fragment key={idx}>
                                          {imir}
                                          {idx < item.merged_imir_no.length - 1 && <br />}
                                        </React.Fragment>
                                      ))
                                    : item?.irn_no || "-"}
                                </td>
