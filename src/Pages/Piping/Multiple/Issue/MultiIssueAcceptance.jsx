...
const MultiIssueAcceptance = () => {
    ...
    const handleSaveClick = () => {
        ...
        const heat_nos = editFormData.heat_no || [];

        if(issuedQty < 0 || isNaN(issuedQty)) {
            toast.error("Issued Qty must be greater than 0.");
            return;
        }

        if(heat_nos.length === 0) {
            toast.error("Please select at least one Heat No.");
            return;
        }
    ...
    const handleSubmit = () => {
        ...
        // ✅ VALIDATE: Every row must have imir_no selected
        // ✅ VALIDATE: Every row must have is_accepted defined (row must be saved)
        const missingAccepted = tableData.find((item) => {
            const hasImir =
                Array.isArray(item.imir_no) && item.imir_no.length > 0;

            const hasHeatno =
                Array.isArray(item.heat_no) && item.heat_no.length > 0;

            const isAcceptedMissing =
                item.is_accepted === undefined || item.is_accepted === null;

            return hasImir && isAcceptedMissing && hasHeatno;
        });
        ...
        bodyFormData.append("isFitUp", data?.isFitUp);
        bodyFormData.append("isDispatch", data?.isDispatch);
        bodyFormData.append("isPainting", data?.isPainting);


        axios.post(`${V_URL}/user/manage-issue-acceptance-piping`, bodyFormData, {
            headers: {
                                                ...
                                                <tbody>
                                                    {commentsData?.map((elem, i) => (
                                                        <tr key={elem?._id}>
                                                            ...
                                                            <td>{elem?.required_qty}</td>
                                                            <td>{elem?.extra_qty}</td>
                                                            <td>{elem?.total_requested_qty}</td>
                                                            {editRowIndex === i ? (
                                                                <>
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        <MultiSelect
                                                                            ...
                                                                            options={getStockOptionsForItem(
                                                                                elem?.material_item_id?.item?._id
                                                                            )}
                                                                            onChange={(e) =>
