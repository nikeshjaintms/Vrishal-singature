...
import { clearIssueOfferItems,getIssueReturnOfferTablePiping } from "../../../../Store/Piping/IssueReturn/getIssueReturnOfferTable";
...
useEffect(() => {
  if (selectedCategory && selectedCategory.length > 0) {
    ...
    dispatch(getIssueReturnOfferTablePiping({
      // category_id: selectedCategory,
      project_id
    }));
  } else {
...
console.log("categoryData",categoryData);
  ...
  const offerData = useSelector((state) => state?.getIssueReturnOfferTablePiping?.user?.data);
...
const handleAddToIssueArr = async (row) => {
  ...
  try {
    ...
    const res = await axios.post(
      `${V_URL}/user/manage-material-issue-return-piping`,
      body,
      {
        headers: {
    ...
    if (res.data.success) {
      ...
      dispatch(
        getIssueReturnOfferTablePiping({
...
const handleRemoveMaterialRow = async (index) => {
  ...
  try {
    const res = await axios.delete(
      `${V_URL}/user/delete-material-issue-offer-return-piping`,
      {
        data: {
          offer_id: row._id,
...
for (let i = 0; i < validOffers.length; i++) {
  ...
  remarks: item.remarks || "",
    ...
    const res = await axios.post(
      `${V_URL}/user/generate-material-issue-return-piping`,
      body,
      {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("PAY_USER_TOKEN"),
    ...
    if (res.data.success) {
      ...
      navigate("/piping/user/issue-return-note", { replace: true });
    } else {
          ...
          <PageHeader
            breadcrumbs={[
              ...
              },
              {
                ...
                link: "/piping/user/issue-return-note",
                active: false,
              },
