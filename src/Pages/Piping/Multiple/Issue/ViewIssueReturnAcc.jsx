...
import { getMultipleIssueReturnPiping } from '../../../../Store/Piping/IssueReturn/getMultipleIssueReturnPiping';
import { getPipingStockReportList } from "../../../../Store/Piping/Stock/getPipingStockReportList";
import { getIssueReturnAcceptancePiping } from "../../../../Store/Piping/IssueReturnAcceptance/getIssueReturnAcceptancePiping";
...
console.log("data edit mode",data);
    useEffect(() => {
        dispatch(getMultipleIssueReturnPiping());
        dispatch(getPipingStockReportList());
        dispatch(getIssueReturnAcceptancePiping());
    }, [dispatch]);

    const stockReportData = useSelector((state) => state.getPipingStockReportList?.user?.data);
    const issueReqData = useSelector((state) => state.getMultipleIssueReturnPiping?.issues?.data);
...
const handleSaveClick = () => {
    ...
    const returnQty = Number(updatedData[dataIndex].return_qty);
    const scrapQty = Number(updatedData[dataIndex].scrap_qty);
...
const handleSubmit = () => {
 ...
 const filteredData = tableData.map(item => ({
    ...
    iss_used_qty:
        Number(item?.iss_used_qty),

    iss_balance_qty:
        Number(item?.iss_balance_qty),
  ...
  axios.post(`${V_URL}/user/manage-material-issue-return-acceptance-piping`, body, {
    headers: {
  ...
  .then(res => {
    ...
    navigate("/piping/user/issue-return-management");
  })
            ...
            <div className="page-wrapper">
                <div className="content">
                    <PageHeader
                        ...
                        { name: "Issue Return List", link: "/piping/user/issue-return-management", active: false },
                                            ...
                                            <table className="table border-0 custom-table comman-table mb-0">
                                                ...
                                                <tbody>
                                                    {commentsData?.map((elem, i) => (
                                                        <tr key={elem?._id}>
                                                            <td>{i + 1}</td>                                           
                                                             <td>{elem?.item_id?.item_name || '-'}</td>
                                                            <td>{elem?.item_id?.item_description || '-'}</td>
                                                            <td>{elem?.item_id?.size1?.name || '-'}</td>
                                                            <td>{elem?.item_id?.thickness1?.name }</td>
                                                    
                                                             <td>{elem?.item_id?.size2?.name || '-'}</td>
                                                            <td>{elem?.item_id?.thickness2?.name || '-'}</td>
                                                        
                                                            <td>{elem?.item_id?.material_grade }</td>
                                                           
                                                            <td>{elem?.item_id?.uom?.name || '-'}</td>
                                    ...
                                    <div className="col-12 text-end">
                                        <div className="doctor-submit text-end">
                                            <button
                                                ...
                                                onClick={() => navigate('/piping/user/issue-return-management')}
                                            >
                                                Back
                                            </button>
                                        </div>
