...
const Drawing = () => {
  ...
  useEffect(() => {
    const findProject = projectData?.find(
      (pro) => pro?._id === localStorage.getItem("U_PROJECT_ID")
    );
  ...
  const getDrawing = () => {
    ...
    const myurl = `${V_URL}/user/get-piping-drawing?page=${currentPage}&limit=${limit}&project=${projectId}&search=${search}`;
    ...
    axios({
      ...
      .then((response) => {
        if (response.data.success === true) {
          ...
          const pagination = Array.isArray(resData)
  ...
  const handleDelete = (id, title) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const myurl = `${V_URL}/user/delete-drawing-piping`;
  ...
  const handleSave = () => {
    if (validation()) {
      const myurl = `${V_URL}/user/issue-piping-drawing`;
  ...
  const handleShow = (id, elem) => {
    ...
    if (elem?.draw_receive_date) {
      const formattedMinDate = new Date(elem.draw_receive_date)
        .toISOString()
        .split("T")[0];
...
// const isEditDisabled = (drawing) => {
//   const spools = drawing?.joint_wise_entries || [];

//   if (spools.length === 0) return false;

//   const allSpoolsLocked = spools.every((spool) => {
//     // Fitup offer generated
//     if (spool.is_generate_fitUp_offer) {
//       return true;
//     }
...
//         const jointNo = (item.joint_no || "").toUpperCase();

//         return (
//           !jointNo.startsWith("FW") &&
//           !jointNo.startsWith("FJ")
//         );
//       }) || [];

//     // Spool locked if ANY valid joint added in fitup
//     return validJoints.some(
//       (item) => item.is_added_fitUp_table === true
//     );
//   });

//   if (drawing.status === 0 || drawing.status === 1) {
//     return false;
//   }

//   if (drawing.status === 2) {
//     return allSpoolsLocked;
//   }
                        ...
                        <tbody>
                          {commentsData?.map((elem, i) => (
                            ...
                            >
                              ...
                              {localStorage.getItem("ERP_ROLE") === PLAN && (
                                <td>
                                  {(elem.status === 1) && (
                                    <input
                                      type="checkbox"
                                      checked={selectedDrawings.includes(
                                        elem._id
                                      )}
                                      onChange={() =>
                                        handleCheckboxChange(elem._id)
                                      }
                                    />
                                  )}
                                </td>
                              ...
                              <td>
                                {moment(elem?.master_updation_date).format(
                                  "DD-MM-YYYY HH:mm"
                                )}
                              </td>
                              ...
                              <td>
                                {elem.piping_class
                                  ? elem.piping_class.PipingClass
                                  : "-"}
                              </td>
                              <td>
                                {moment(elem?.drawing_receive_date).format(
                                  "DD-MM-YYYY"
                                )}
                              </td>
                              ...
                              <td>
                                {elem?.issued_date
                                  ? moment(elem?.issued_date).format(
                                    "DD-MM-YYYY"
                                  )
                                  : "-"}
                              </td>
                                  ...
                                  <>
                                    <div className="add-group">
                                      <input
                                        ...
                                        accept=".pdf"
                                        onChange={(e) =>
                                          handleUploadPdf(e, elem?._id)
                                        }
                                        style={{ display: "none" }}
                                        id={`upload-pdf-${elem?._id}`}
                                      />
                                      <label
                                        htmlFor={`upload-pdf-${elem?._id}`}
                                  ...
                                  <div className="dropdown-menu dropdown-menu-end">
                                    ...
                                    {!isEditDisabled(elem) && (
                                      <button
                                        ...
                                        className="dropdown-item"
                                        onClick={() =>
                                          navigate("/piping/user/manage-drawing", { state: elem })
                                        }
                                      >
                                        <i className="fa-solid fa-pen-to-square m-r-5"></i>
                                        Edit
            ...
            <div className="row align-items-center mt-2">
              <div className="col-3">
                <label className="col-form-label">
                  Issue To <span className="login-danger">*</span>{" "}
                </label>
              </div>
              <div className="col-9">
                <select
                  className="form-control"
                  value={drawIssue.name}
                  name="name"
                  onChange={handleChange}
                >
                  <option vlaue="">Select Contractor </option>
                  {contractorData?.map((elem) => (
                    <option value={elem?.conId._id} key={elem?.conId?._id}>
                      {elem?.conId?.name}
                    </option>
                  ))}
                </select>
            ...
            <div className="row align-items-center mt-2">
              ...
              <div className="col-9">
                <input
                  ...
                  value={
                    drawIssue.date
                      ? new Date(drawIssue.date).toISOString().split("T")[0]
                      : ""
                  }
                  name="date"
                  onChange={handleChange}
