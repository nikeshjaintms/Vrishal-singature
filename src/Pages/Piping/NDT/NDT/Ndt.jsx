...
const Ndt = () => {
    ...
    useEffect(() => {
        const fetchData = () => {
            if (disable === true) {
                dispatch(getUserMultiNdtMaster({ status: '' }));
                setDisable(false);
            }
        }
        fetchData();
    ...
    const entity = useSelector((state) => state?.getUserMultiNdtMaster?.user?.data);

    const commentsData = useMemo(() => {
        let computedComments = entity;
        if (search) {
            computedComments = computedComments.filter((ndt) => {
                const uniqueAssemblyNos = [
                    ...new Set(ndt?.items?.map(e => e?.grid_item_id?.drawing_id?.assembly_no).filter(Boolean))
                ].join(", ");

                const uniqueAssemblyQty = [
                    ...new Set(ndt?.items?.map(e => e?.grid_item_id?.drawing_id?.assembly_quantity).filter(Boolean))
                ];
                const uniqueDrawingNo = [
                    ...new Set(ndt?.items?.map(e => e?.grid_item_id?.drawing_id?.drawing_no).filter(Boolean))
                ];
