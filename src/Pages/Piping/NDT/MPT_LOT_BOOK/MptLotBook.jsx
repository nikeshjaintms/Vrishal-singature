...
import { getMultiMptLotPiping } from '../../../../Store/Piping/Ndt/MPT-LOT/MPT';
...
const MptLotBook = () => {
    ...
    const entityArray = useSelector((state) => state?.getMultiMptLotPiping?.offers);
    const totalCount = useSelector((state) => state?.getMultiMptLotPiping?.pagination?.total);
    ...
    const handleDownload = (elem) => {
        ...
        PdfDownloadErp({ apiMethod: 'post', url: 'piping/download-mpt-lot-pdf', body: bodyFormData });
    }

    const handleEdit = (elem) => {
        navigate('/piping/user/manage-mpt-lot-book-management', { state: { data: elem, is_view: false, is_edit: true } });
    }

    const handleView = (elem) => {
        navigate('/piping/user/manage-mpt-lot-book-management', { state: { data: elem, is_view: true, is_edit: false } });
    }
                        ...
                        <div className="row">
                            <div className="col-sm-12">
                                <ul className="breadcrumb">
                                    ...
                                    <li className="breadcrumb-item active">MPT Lot Book List</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                                                        ...
                                                        <div className="doctor-search-blk">
                                                            ...
                                                            <div className="add-group">
                                                                {localStorage.getItem('ERP_ROLE') === QC &&
                                                                    <Link to="/piping/user/manage-mpt-lot-book-management"
                                                    ...
                                                    {commentsData?.map((elem, i) => {
                                                        ...
                                                        return (
                                                            <tr key={elem?._id}>
                                                                ...
                                                                <td>{elem?.items?.map((item) => item?.ndt_percentage).join(", ")}</td>
                                                                <td>
                                                                    {elem.status === 1 ? (
