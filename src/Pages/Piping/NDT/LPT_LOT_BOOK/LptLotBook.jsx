...
import { getMultiLptLotPiping } from '../../../../Store/Piping/Ndt/LPT-LOT/LptLot';
...
useEffect(() => {
    ...
    const entityArray = useSelector((state) => state?.getMultiLptLotPiping?.offers);
    const totalCount = useSelector((state) => state?.getMultiLptLotPiping?.pagination?.total);
    ...
    const handleDownload = (elem) => {
        ...
        bodyFormData.append('report_no', elem.lpt_lot_number);
        bodyFormData.append('print_date', true);
        PdfDownloadErp({ apiMethod: 'post', url: 'piping/download-lpt-lot-pdf', body: bodyFormData });
    }

    const handleEdit = (elem) => {
        navigate('/piping/user/manage-lpt-lot-book-management', { state: { data: elem, is_view: false, is_edit: true } });
    }

    const handleView = (elem) => {
        navigate('/piping/user/manage-lpt-lot-book-management', { state: { data: elem, is_view: true, is_edit: false } });
    }
                        ...
                        <div className="row">
                            <div className="col-sm-12">
                                <ul className="breadcrumb">
                                    ...
                                    <li className="breadcrumb-item active">LPT Lot Book List</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                                                        ...
                                                        <div className="doctor-search-blk">
                                                            ...
                                                            <div className="add-group">
                                                                {localStorage.getItem('ERP_ROLE') === QC &&
                                                                    <Link to="/piping/user/manage-lpt-lot-book-management"
          ...
          return (
            <tr key={elem?._id || i}>
              ...
              <td>{elem?.lpt_lot_number}</td>
                <td>{[...new Set(elem?.items?.map(i => i?.drawing_no).filter(Boolean))].join(", ")}</td>
                                                        <td>{[...new Set(
  elem?.items?.map(i => i?.spool_no).filter(Boolean)
)].join(", ")}
</td>
