...
const ManageLptOffer = () => {
    ...
    const getThickness = (transactionId, itemId) => {
        const findStock = stockData?.find((stock) => stock?.itemId === itemId);
        return `T/B: ${findStock?.accepted_topBottom_thickness} W: ${findStock?.accepted_width_thickness} N: ${findStock?.accepted_normal_thickness}` || '-';
    };

    useEffect(() => {
        if (location.state?._id) {
            const processedData = location.state?.items.map((item) => ({
                ...item,
                profile_size: item?.transaction_id?.itemName?.name || '',
                thickness: getThickness(item?.transaction_id?._id, item?.transaction_id?.itemName?._id),
                remarks: '',
            }));
            setTableData(processedData);
        }
    }, [location.state, stockData]);
    ...
    const handleEditClick = (index, row) => {
        ...
        setEditFormData({
            thickness: row.thickness || getThickness(row.transaction_id?._id, row.transaction_id?.itemName?._id),
            profile_size: row.profile_size || row.transaction_id?.itemName?.name,
            remarks: row.remarks,
        });
    }
    ...
    const handleSubmit = () => {
        ...
        updatedData.forEach(item => {
            ...
            if (item.profile_size === '' || item.profile_size === undefined) {
                isValid = false;
                toast.error(`Please enter profile size for ${item.transaction_id.itemName.name}`);
            }
        });
        ...
        axios({
            ...
            if (response.data.success === true) {
                ...
                navigate('/piping/user/lpt-offer-management');
            }
            setDisable(false);
        }).catch((error) => {
                        ...
                        <div className="row">
                            <div className="col-sm-12">
                                <ul className="breadcrumb">
                                    ...
                                    <li className="breadcrumb-item"><Link to="/piping/user/lpt-offer-management">Liquid Penetrant Testing Offer List</Link></li>
                                    <li className="breadcrumb-item"><i className="feather-chevron-right"></i></li>
                                    <li className="breadcrumb-item active">{data?._id ? 'Edit' : 'Add'} Liquid Penetrant Testing Offer List</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <form>
                                        <div className="col-12">
                                            <div className="form-heading">
                                                <h4>Manage Magnetic Particle Testing Offer Details</h4>
                                            </div>
                                        </div>
                                                        ...
                                                        {(data?.status === 1 || data?.status === 4) ? (
                                                            <>
                                                                {editRowIndex === i ? (
                                                                    <>
                                                                        <td>
                                                                            <input className='form-control' type='text' value={editFormData?.profile_size} onChange={handleEditFormChange} name='profile_size' />
                                                                        </td>
                                                                        <td>
                                                                    ...
                                                                    <>
                                                                        <td onClick={() => handleEditClick(i, elem)}>{elem?.profile_size || elem?.transaction_id?.itemName?.name}</td>
                                        ...
                                        <div className="doctor-submit text-end">
                                            {localStorage.getItem('ERP_ROLE') === QC ? (
                                                <button type="button" className="btn btn-primary submit-form me-2" onClick={handleSubmit} disabled={disable}>
                                                    {disable ? "Processing..." : "Generate LPT Offer"}
                                                </button>
                                            ) : (
                                                <button type="button" className="btn btn-primary submit-form me-2" onClick={() => navigate('/piping/user/lpt-offer-management')}>
                                                    Back
                                                </button>
                                            )}
                                        </div>
