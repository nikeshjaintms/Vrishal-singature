...
import { getProjectFrontAvailabilitySummary } from "../../../Store/Piping/Drawing/getProjectFrontAvailabilitySummary";
...
const ProjectFrontAvailabilitySummary = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const [disable, setDisable] = useState(true);
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
const [priorityChanges, setPriorityChanges] =
  useState({});
...
const [statusFilter, setStatusFilter] = useState("All");
  ...
  (state) => state?.getProjectFrontAvailabilitySummary?.user || {}
);

 const pagination = useSelector(
  (state) => state?.getProjectFrontAvailabilitySummary?.user?.pagination || {}
);
...
const priorityOptions = Array.from(
  ...
  (_, i) => ({
    ...
    label: `P-${String(i + 1).padStart(3, "0")}`,
  })
);
  ...
  useEffect(() => {
    dispatch(
      getProjectFrontAvailabilitySummary({
        page: currentPage,
        limit,
        search,
        status: statusFilter,
...
const processedRows = useMemo(() => {
  ...
  //   DownloadXlsx({
  //     apiMethod: "post",
  //     url: "excel-project-front-availability-summary-download",
  //     body: bodyFormData,
  //     fileName: "Project_Front_Availability_Summary",
  //   });
  // };
  ...
  try {
    const response = await axios.post(
      `${V_URL}/user/excel-project-front-availability-summary-download`,
      {
        project: localStorage.getItem("U_PROJECT_ID"),
      },
      {
    ...
    link.setAttribute(
      "download",
      "Project_Front_Availability_Summary.xlsx"
    );
...
const handleSavePriority = async (
  ...
  try {
    await axios.post(
      `${V_URL}/user/update-drawing-priority`,
      {
...
{priorityOptions.map((option) => (
  ...
  <button
    ...
    title="Save Priority"
    disabled={!priorityChanges[row._id]}
    onClick={() =>
      handleSavePriority(
        row._id,
        priorityChanges[row._id]
...
export default ProjectFrontAvailabilitySummary;
