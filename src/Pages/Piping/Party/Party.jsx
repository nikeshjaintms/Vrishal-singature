...
import { MaterialCoordinatorAuth, PlannerQcAuth } from '../../../Routes/Users/Auth/AuthGuard';
...
const Party = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (disable === true) {
        try {
          await dispatch(getAdminParty({ storeType: '', is_main: false }))
          setDisable(false);
        } catch (error) {
          console.log(error, '!!')
          setDisable(false);
    ...
    if (search) {
      computedComments = computedComments.filter(
        (party) =>
          ...
          party.phone?.toString().toLowerCase()?.includes(search?.toLowerCase()) ||
          party.address?.toLowerCase()?.includes(search?.toLowerCase()) ||
          party.gstNumber?.toLowerCase()?.includes(search?.toLowerCase()) ||
          party.partyGroup.name?.toLowerCase()?.includes(search?.toLowerCase())
      );
    }
  ...
  const handleDelete = (id, title) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const myurl = `${V_URL}/user/delete-party`;
            ...
            <div className="row">
              <div className="col-sm-12">
                <ul className="breadcrumb">
                  ...
                  <li className="breadcrumb-item active">Stack Holder List</li>
                </ul>
              </div>
            </div>
          </div>
                          ...
                          <div className="doctor-table-blk">
                            ...
                            <div className="doctor-search-blk">
                              ...
                              <div className="add-group">
                                <PlannerQcAuth>
                                  <Link to="/piping/user/manage-party"
                        ...
                        <tbody>
                          {commentsData?.map((elem, i) =>
                            <tr key={elem?._id}>
                              ...
                              <td>{elem?.name}</td>
                              <td>{elem?.partyGroup?.name}</td>
                              <td>{elem?.party_tag_id?.name}</td>
                              <td>{elem?.email === '' ? '-' : elem?.email}</td>
                              <td>{elem?.phone || '-'}</td>
                              <td>{elem?.gstNumber === '' ? '-' : elem?.gstNumber}</td>
                              <td>{elem?.address},{elem?.city},{elem?.state}-{elem?.pincode === 0 ? '' : elem?.pincode}</td>
                                ...
                                <td className="text-end">
                                  <div className="dropdown dropdown-action">
                                    ...
                                    <div className="dropdown-menu dropdown-menu-end">
                                      <button type='button' className="dropdown-item" onClick={() => navigate('/piping/user/manage-party', { state: elem })}><i
