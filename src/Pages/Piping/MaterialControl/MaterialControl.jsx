...
// Function to safely extract a property from a nested object
...
const MaterialControl = () => {
    ...
    // State to hold the flattened and paginated data for the table
   ...
   useEffect(() => {
    ...
    const mtoRows = entity.map((mto) => ({
        ...
        // ✅ Include the full lineno_drawingno array
        lineno_drawingno: mto.lineno_drawingno?.map((line) => ({
            ...line,
            drawings: line.drawings?.map((d) => ({
                ...d,
                drawing_id: d.drawing_id, // ✅ keep drawing_id
            })),
        })) ?? [],
    }));
                                                    ...
                                                    <div className="doctor-table-blk">
                                                        ...
                                                        <div className="doctor-search-blk">
                                                            ...
                                                            <div className="add-group">
                                                                <Link to="/piping/user/manage-material-control"
                                                ...
                                                <tbody>
                                                    {tableData.length === 0 ? (
                                                        ...
                                                        paginatedTableData.map((row, i) => (
                                                            <tr key={row._id + i}> {/* Using a composite key for uniqueness */}
                                                                ...
                                                                {localStorage.getItem('ERP_ROLE') === M_CON && (
                                                                    <td>
                                                                    {row.send_to_pr === true  || row.status === 2 ? (
                                                                        ''
                                                                    ): (
                                                                        <span style={{ cursor: "pointer" }} onClick={() => handleSendPr(row?._id, row?.mto_no)}>
                                                                            <SendHorizontal />
                                                                        </span>
                                                                    )}
                                                                    </td>
                                                                ...
                                                                <td>
                                                                    {row.send_to_qc === true && row.status !== 2 && row.status !== 3 && row.status !== 4  ? (
                                                                    <button
                                                                        className="btn btn-primary btn-sm"
                                                                        onClick={() =>
                                                                        navigate("/piping/user/fim-packing-verification", { state: row })
                                                                        }
                                                                    >
                                                                        Verify
                                                                    </button>
                                                                ...
                                                                <td className="text-end">
                                                                    <div className="dropdown dropdown-action">
                                                                        ...
                                                                        <div className="dropdown-menu dropdown-menu-end">
                                                                            {/* Note: View/Edit/Delete/Download actions use the parent MTO ID: row._id */}
                                                                            ...
                                                                            <button
                                                                                ...
                                                                                onClick={() => navigate("/piping/user/manage-material-control", { state: row })}
                                                                            >
                                                                                <i className="fa-solid fa-pen m-r-5"></i> Edit
                                                                            </button>
                                                                            <button className="dropdown-item" onClick={() => handleDelete(row._id, row.project_name)}>
