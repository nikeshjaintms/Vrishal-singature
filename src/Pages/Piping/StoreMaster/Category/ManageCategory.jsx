...
import { addItemCategory } from '../../../../Store/Piping/ItemCategory/ManageItemCategory';
  ...
  useEffect(() => {
    if (location.state) {
      setCategory(location.state?.name);
       setSize2Thickness2Required(location.state?.size2_thickness2_required);
      setSelectValue(location.state?.status);
    }
  }, [location.state]);

  const [category, setCategory] = useState('');
const [size2Thickness2Required, setSize2Thickness2Required] = useState(false);
  ...
  const handleSubmit = () => {
    if (validation()) {
      ...
      formData.append('size2_thickness2_required', size2Thickness2Required);
      if (data?._id) {
        formData.append('id', data?._id);
        formData.append('status', selectValue);
      }

      dispatch(addItemCategory(formData))
        .then((res) => {
          console.log(res, 'REs');

          if (res.payload.success === true) {
            navigate('/piping/user/category-management');
            setCategory('');
          }
          setDisable(false)
        }).catch((error) => {
            ...
            <div className="row">
              <div className="col-sm-12">
                <ul className="breadcrumb">
                  ...
                  <li className="breadcrumb-item"><Link to="/piping/user/category-management">Item Category List</Link></li>
                  <li className="breadcrumb-item"><i className="feather-chevron-right"></i></li>
...
<div className='col-12 col-md-4 col-xl-4'>
                          <div className="cardNum">
                            <div className="mb-3">
                              <label htmlFor="fileUpload" className="form-label">  Size 2 & Thickness 2 Required?</label>
                              <div className="form-check form-switch">
                                <input className="form-check-input" type="checkbox" role="switch"  checked={size2Thickness2Required}
      onChange={(e) => setSize2Thickness2Required(e.target.checked)}
      value={size2Thickness2Required}
       />
                              </div>
                            </div>
                          </div>
