...
import { addThickness } from '../../../../Store/Piping/Thickness/ManageThickness';
const ManageThickness = () => {
    ...
    useEffect(() => {
        if (location.state) {
            setThickness(location.state?.name);
    ...
    const handleSubmit = () => {
        if (validation()) {
            ...
            formData.append('name', thickness);
            if (data?._id) {
                formData.append('id', data?._id);
                formData.append('status', selectValue);
            }

            dispatch(addThickness(formData))
                .then((res) => {
                    console.log(res, 'REs');

                    if (res.payload.success === true) {
                        navigate('/piping/user/thickness-management');
                        setThickness('');
                    }
                    setDisable(false)
                }).catch((error) => {
                        ...
                        <div className="row">
                            <div className="col-sm-12">
                                <ul className="breadcrumb">
                                    ...
                                    <li className="breadcrumb-item"><Link to="/piping/user/thickness-management">Thickness List</Link></li>
                                    <li className="breadcrumb-item"><i className="feather-chevron-right"></i></li>
                                    <li className="breadcrumb-item active">{data?._id ? 'Edit' : 'Add'} Thickness</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <form>
                                        <div className="col-12">
                                            <div className="form-heading">
                                                <h4>{data?._id ? 'Edit' : 'Add'} Thickness Details</h4>
                                            </div>
                                        </div>
...
export default ManageThickness