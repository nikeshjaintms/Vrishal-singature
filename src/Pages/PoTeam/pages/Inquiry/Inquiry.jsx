...
import { getAllInquiries } from '../../../../Store/PoTeam/Inquiry/Inquiry'; // redux slice you created
...
const loading = useSelector((state) => state.getInquiry?.loading || false);
  ...
  const handleDeleteInquiry = async (id, inquiryNo) => {
    ...
    try {
      ...
      const response = await axios.delete(`${V_URL}/user/inquiry/delete-inquiry`, {
  ...
  const handleDownloadPDF = async (id) => {
    try {
      ...
      const response = await axios.post(
        `${V_URL}/user/inquiry/download-inquiry-pdf`,
        { id },
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    const handleDownloadExcel = async (id) => {
      try {
        ...
        const response = await axios.post(
          `${V_URL}/user/inquiry/download-inquiry-excel`,
          { id, project_id },
          {
            headers: {
...
const handleSendSelected = async () => {
  ...
  try {
    const token = localStorage.getItem("PAY_USER_TOKEN");
    const response = await axios.post(
      `${V_URL}/user/inquiry/send-multiple-inquiry`,
                            ...
                            <div className="doctor-search-blk">
                              ...
                              <div className="add-group">
                                {localStorage.getItem('PO_ROLE') === Procurement && (
                                  <Link to="/material-po/manage-inquiry" className="btn btn-primary add-pluss ms-2">
                                ...
                                <td className="text-end">
                                  <div className="dropdown dropdown-action">
                                    ...
                                    <div className="dropdown-menu dropdown-menu-end">
                                      <button className="dropdown-item" onClick={() => navigate("/material-po/view-inquiry", { state: row })}>
                                        <i className="fa-solid fa-eye m-r-5"></i> View
                                      </button>
                                      {row.sendPO === false && (
                                      <>
                                      {localStorage.getItem('PO_ROLE') === Procurement && row.sendPO === false && (
                                        <button className="dropdown-item" onClick={() => navigate("/material-po/manage-inquiry", { state: row })}>
