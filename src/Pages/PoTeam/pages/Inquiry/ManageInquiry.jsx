...
const ManageInquiry = () => {
  ...
  // Redux selectors
  const prList = useSelector((state) => state.getProcurementRequest?.list?.data || []);
  const inquiryList = useSelector((state) => state.getInquiry?.list?.data || []);
  const allTerms = useSelector((state) => state.terms?.list?.data || []);
  ...
  // Fetch initial data
  useEffect(() => {
    dispatch(getAllInquiries({ project: localStorage.getItem("U_PROJECT_ID") }));
    ...
    if (editData?._id) {
      ...
      const mappedItems = (editData.items || []).map(it => {
        ...
        return {
          ...
          uom: (typeof it.uom === 'object' ? it.uom?.name : it.uom) || it.item?.unit?.name || "",
          approvedMake: approvedMake,
          manufacture: manufactureIds,
          qty: it.qty || 0,
          requiredSize: it.requiredSize || it.item?.sectionLengthOrDimensions || "",
          remarks: it.remarks || "",
        };
      });
  ...
  const handlePoSelect = (e) => {
    ...
    const items = (pr.items || [])
      ...
      .map(it => ({
        ...
        requiredSize: it.requiredSize || it.sectionLengthOrDimensions || "",
        remarks: it.remarks || "",
      }));
  ...
  const mergedInquiryItems = useMemo(() => {
    ...
    inquiryItems.forEach(item => {
      const key = `${item.item}_${item.material_Grade}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${V_URL}/user/inquiry/manage-inquiry`, payload, {
      ...
      if (res.data.success) {
        ...
        navigate("/material-po/inquiry-management");
      } else {
        ...
        <div className="content">
          <div className="page-header">
            <ul className="breadcrumb">
              ...
              <li className="breadcrumb-item"><Link to="/material-po/inquiry-management">Inquiry</Link></li>
              <li className="breadcrumb-item"><i className="feather-chevron-right" /></li>
