...
const PipingProcurementRequestList = () => {
  ...
  // ---------------- Redux selectors ----------------
  const entity = useSelector((state) => state.getPipingProcurementRequest?.list);
  ...
  const handleSendToGr = async (id, prNo) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${V_URL}/user/pr/send-inquiry-piping`,
        { id },
  ...
  const handleSendMultipleToInquiry = async () => {
    ...
    try {
      const token = localStorage.getItem("PAY_USER_TOKEN");
      const response = await axios.post(
        `${V_URL}/user/pr/send-multiple-inquiry-piping`,
        { ids: selectedPRs },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleDeletePR = async (id, prNo) => {
    ...
    try {
      const token = localStorage.getItem("PAY_USER_TOKEN");
      const response = await axios.delete(`${V_URL}/user/pr/delete-procurement-request-piping`, {
        headers: { Authorization: `Bearer ${token}` },
        data: { id },
      });
  ...
  const handleDownloadPDF = async (id) => {
    try {
      const token = localStorage.getItem("PAY_USER_TOKEN");
      const response = await axios.post(
        `${V_URL}/user/pr/download-pr-pdf-piping`,
        { pr_id: id },
        { headers: { Authorization: `Bearer ${token}` } }
      );
                          ...
                          <div className="doctor-table-blk">
                            ...
                            <div className="doctor-search-blk">
                              ...
                              {CENTRAL_PLAN === localStorage.getItem('PO_ROLE') && (
                                <Link to={PO_ROUTE_URLS.PIPING_MANAGE_PR} className="btn btn-primary add-pluss ms-2">
                                ...
                                <td className="text-end">
                                  <div className="dropdown dropdown-action">
                                    ...
                                    <div className="dropdown-menu dropdown-menu-end">
                                      <button className="dropdown-item" onClick={() => navigate(PO_ROUTE_URLS.PIPING_PR_VIEW, { state: row })}>
                                        <i className="fa-solid fa-eye m-r-5"></i> View
                                      </button>
                                      {row.sendInquiry === false && 
                                     (<>
                                     {CENTRAL_PLAN === localStorage.getItem('PO_ROLE') && (
                                       <button className="dropdown-item" onClick={() => navigate(PO_ROUTE_URLS.PIPING_MANAGE_PR, { state: row })}>
...
export default PipingProcurementRequestList;
