...
import { UpdateMioBalance } from '../../../../../../Store/MutipleDrawing/MultiFinalCoat/UpdateMioBalance';
...
const FinalCoatModel = ({ showItem, handleCloseModal, title, finalCoatdata, paintNo, dispatch_site, report_no }) => {
    ...
    console.log(finalCoatdata, "finalCoatdata");    

    useEffect(() => {
        const finalData = finalCoatdata?.items?.map((e) => ({
            ...e,
            mio_used_grid_qty: parseInt(e?.mio_used_grid_qty) - (parseInt(e.moved_next_step) || 0),
            original_qty: (parseInt(e?.mio_used_grid_qty) || 0),
            max_qty: (parseInt(e?.mio_used_grid_qty) || 0) - (parseInt(e.moved_next_step) || 0),
            fc_used_grid_qty: e?.fc_used_grid_qty || '',
            mio_balance_grid_qty: e?.mio_balance_grid_qty || '',
            ...
            prevItemData.map((item, idx) => {
                if (idx === index) {
                    ...
                    if (newValue === "" || (newValue >= 0 && newValue <= final_qty)) {
                        ...
                        return {
                            ...item,
                            fc_used_grid_qty: newValue,
                            mio_used_grid_qty: updatedQty,
                            fc_balance_grid_qty: updatedQty,
                        };
                    }
                }
                return item;
    ...
    const handleSave = async () => {
        const updatedArray = itemData?.filter((it) => it?.fc_used_grid_qty > 0);
        ...
        try {
            await dispatch(UpdateMioBalance({ bodyFormData: UpdateDispatch }));
            await dispatch(manageFinalCoatOffer({ bodyFormData: ManageDispatch }))
            dispatch(getMultiFinalCoat({ "paint_system_id": paintNo }))
            const payload = {
                                            ...
                                            <tbody>
                                                {commentsData?.map((elem, index) =>
                                                    <tr key={index}>
                                                        ...
                                                        <td>{elem?.grid_no || '-'}</td>
                                                        <td>{elem?.grid_qty || '-'}</td>
                                                        <td>{elem?.mio_used_grid_qty}</td>
                                                        <td>
                                                            <input
                                                                className='form-control' style={{ padding: "4px", minHeight: '15px' }}
                                                                type="number"
                                                                value={elem.fc_used_grid_qty}
