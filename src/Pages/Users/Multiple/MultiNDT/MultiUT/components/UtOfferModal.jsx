...
const UtOfferModal = ({ title, tableObj, modalOpen, handleCloseModal }) => {

    // console.log(tableObj, 'mod')
    ...
    const [utTableData, setUtTableData] = useState([]);
    ...
    const fetchUtTable = () => {
        ...
        axios({
            ...
            if (response?.data?.success === true) {
                setUtTableData(response?.data?.data?.items)
            }
        }).catch((error) => {
            console.log(error);
        })
    ...
    useEffect(() => {
        if (tableObj?._id) {
            ...
            const updatedTableData = tableObj?.items?.map((item) => {
                ...
                const filterRTTable = utTableData?.filter((rt) => rt?.ndt_type_id === tableObj?.ndt_type_id?._id && rt?.ndt_master_id === tableObj?.ndt_master_id?._id);


                const matchingRTItems = filterRTTable?.filter((ut) =>
                    ut?.grid_item_id?._id === item?.grid_item_id?._id &&
                    ut?.grid_item_id?.drawing_id?._id === item?.drawing_id &&
                    ut?.deleted === false &&
                    ut?.offer_status !== 5 && ut?.offer_status !== 4
                ) || [];

                const totalMatchingMPTUseQty = matchingRTItems?.reduce((sum, ut) => sum + (ut?.ut_use_qty || 0), 0);
            ...
            setUtItems(updatedTableData);
        }
    }, [tableObj?._id, stockData, utTableData, modalOpen]);

    const commentsData = useMemo(() => {
        let computedComments = utItems;
        if (search) {
            computedComments = computedComments.filter(
                (ut) =>
                    ut.grid_item_id?.drawing_id?.drawing_no?.toLowerCase()?.includes(search?.toLowerCase()) ||
                    ut.grid_item_id?.drawing_id?.assembly_no?.toLowerCase()?.includes(search?.toLowerCase()) ||
                    ut.grid_item_id?.grid_id?.grid_no?.toLowerCase()?.includes(search?.toLowerCase())
            );
        }
    ...
    }, [currentPage, search, limit, utItems]);
    ...
    const handleEditFormChange = (e) => {
        ...
        if (name === 'ut_use_qty') {
            if (value === "") {
                ...
                const maxQty = utItems[editRowIndex]?.offer_used_grid_qty || 0;
    ...
    const handleSaveClick = () => {
        const updatedData = utItems.map((item) => {
            if (item.grid_item_id?.grid_id?.grid_no === utItems[editRowIndex]?.grid_item_id?.grid_id?.grid_no) {
                return { ...item, ...editFormData, ut_use_qty: editFormData.ut_use_qty || "" };
            }
            return item;
        });

        setUtItems(updatedData);
    ...
    const handleSave = () => {
        const updatedData = utItems.filter((it) => it?.ut_use_qty > 0);
        if (updatedData?.length === 0) {
            toast.error('Please add at least one item with non-zero UT Use Qty');
            return
        }
        ...
        const finalArray = updatedData?.map((it) => ({
            ...
            ut_use_qty: it?.ut_use_qty,
            ut_balance_qty: parseInt(it?.offer_used_grid_qty) - parseInt(it?.ut_use_qty),
        ...
        let storedMasterIds = JSON.parse(localStorage.getItem('UT_TYPE_MASTER_IDS')) || [];
        ...
        localStorage.setItem('UT_TYPE_ID', tableObj?.ndt_type_id?._id);
        localStorage.setItem('UT_TYPE_MASTER_IDS', JSON.stringify(storedMasterIds));
        ...
        dispatch(manageRTOfferTable({ bodyFormData: formData })).then((response) => {
            ...
            if (success === true) {
                ...
                const typeId = localStorage.getItem('UT_TYPE_ID');
                const masterId = localStorage.getItem('UT_TYPE_MASTER_IDS');
                dispatch(getMultiUTOfferTable({ typeId, masterId }))
                handleCloseModal();
            }
        })
    }
                                                            ...
                                                            <>
                                                                {editRowIndex === i ? (
                                                                    ...
                                                                    <>
                                                                        ...
                                                                        <td onClick={() => handleEditClick(i, elem)}>{elem?.ut_use_qty || '-'}</td>
                                                                    </>
                                                                )}
                                                                {editRowIndex === i ? (
                                                                    <td>
