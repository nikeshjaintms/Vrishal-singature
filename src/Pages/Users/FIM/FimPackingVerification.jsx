...
import FimQcVerificationTable from "../../../Components/FIMModal/FimQcVerificationTable";
  ...
  const handleSubmitVerification = async () => {
    try {
      ...
      for (let item of items) {
        if (item.manufacture.trim() === "") {
          toast.error("Please fill Manufacture for all items.");
          return;
        }
        if (
          !item.tc_heat_details ||
          item.tc_heat_details.some(
            (d) => d.tc_no.trim() === "" || d.heat_no.trim() === ""
          )
        ) {
          toast.error("Please fill TC No and Heat No for all items.");
          return;
        }
      }
      ...
      const result = await axios.post(`${V_URL}/user/fim/verify-fim-packing`, bodyFormData, {
        headers: {
