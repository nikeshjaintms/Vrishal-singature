...
const FimPackingList = () => {
  ...
  const getPackingLists = async () => {
      ...
      `${V_URL}/user/fim/get-fim-packing-list`,
...
const commentsData = useMemo(() => entity, [entity]);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`/fim/delete-packing-list/${id}`, {
  ...
  try {
    const res = await axios.post(
      `${V_URL}/user/fim/download-fim-packing`,
      { fim_id: elem._id },
      {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("PAY_USER_TOKEN"),
  ...
  const handleDownloadIMIRPdf = async (elem) => {
    try {
      const res = await axios.post(
        `${V_URL}/user/fim/download-fim-data`,
        { fim_id: elem._id },
        {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("PAY_USER_TOKEN"),
...
const handleSendQc = (id, title) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const myurl = `${V_URL}/user/fim/send-fim-to-qc`;
...
const handleDownloadExcel = async (elem) => {
  try {
    const res = await axios.post(
      `${V_URL}/user/fim/download-fim-packing-excel`,
      { fim_id: elem._id },
      {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("PAY_USER_TOKEN"),
                          ...
                          <div className="doctor-table-blk">
                            ...
                            <div className="doctor-search-blk">
                              ...
                              {localStorage.getItem('ERP_ROLE') === M_CON && (
                                 <Link
                                    to="/user/project-store/manage-fim-packing"
                              ...
                              {localStorage.getItem('ERP_ROLE') === M_CON && (
                                  <td>
                                    {elem.send_to_qc === true  || elem.status === 2 ? (
                                      ...
                                      <span style={{ cursor: "pointer" }} onClick={() => handleSendQc(elem?._id, elem?.packing_no)}>
                                          <SendHorizontal />
                                      </span>
                                    )}
                                  </td>
                                ...
                                <td>
                                  {elem.send_to_qc === true && elem.status !== 2  ? (
                                    <button
                                      ...
                                      onClick={() =>
                                        navigate("/user/project-store/fim-packing-verification", { state: elem })
                                      }
                                    >
                                      Verify
                                    </button>
                                  ...
                                  <div className="dropdown-menu dropdown-menu-end">
                                    ...
                                    {elem.send_to_qc === false && localStorage.getItem("ERP_ROLE") === M_CON && (
                                     <button
                                      type="button"
                                      className="dropdown-item"
                                      onClick={() => navigate('/user/project-store/manage-fim-packing', { state: elem })}
                                    >
