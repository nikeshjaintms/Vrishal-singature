...
const WpsMaster = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (disable === true) {
                dispatch(getUserWpsMaster({ status: '' }));
                setDisable(false);
            }
        }
        fetchData();
    ...
    const entity = useSelector((state) => state?.getUserWpsMaster?.user?.data);
    console.log("WPS Master List:", entity);
    ...
    const commentsData = useMemo(() => {
        ...
        if (search) {
            computedComments = computedComments.filter(
                (wps) =>
                    wps?.jointType?.name?.toLowerCase()?.includes(search?.toLowerCase()) ||
                    wps?.wpsNo?.toLowerCase()?.includes(search?.toLowerCase()) ||
                    wps?.weldingProcess?.toLowerCase()?.includes(search?.toLowerCase())
            );
        }
    ...
    const handleDelete = (id, title) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const myurl = `${V_URL}/user/delete-wps-master`;
                                                    ...
                                                    <div className="doctor-table-blk">
                                                        ...
                                                        <div className="doctor-search-blk">
                                                            ...
                                                            <div className="add-group">
                                                                <WeldingAuth>
                                                                    <Link to="/user/project-store/manage-wps-master"
                                                ...
                                                <tbody>
                                                    {commentsData?.map((elem, i) =>
                                                        <tr key={elem?._id}>
                                                            ...
                                                            <td>
                                                                {elem?.jointType?.map((e, i) => `${e?.jointId?.name}`)?.join(', ')}
                                                            </td>
                                                            <td>{elem?.wpsNo}</td>
                                                                ...
                                                                <td className="text-end">
                                                                    <div className="dropdown dropdown-action">
                                                                        ...
                                                                        <div className="dropdown-menu dropdown-menu-end">
                                                                            <button type='button' className="dropdown-item" onClick={() => navigate('/user/project-store/manage-wps-master', { state: elem })}><i
                                                                                className="fa-solid fa-pen-to-square m-r-5"></i>
                                                                                Edit</button>
                                                                            <button type='button' className="dropdown-item" onClick={() => handleDelete(elem?._id, elem.wpsNo)} ><i
