...
const WelderMaster = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (disable === true) {
        dispatch(getUserWelderMaster({ status: '' }));
        setDisable(false);
      }
    }
    fetchData();
  ...
  const entity = useSelector((state) => state?.getUserWelderMaster?.user?.data);
  ...
  const commentsData = useMemo(() => {
    ...
    if (search) {
      computedComments = computedComments.filter(
        (welder) =>
          welder?.name?.toLowerCase()?.includes(search?.toLowerCase()) ||
          welder?.welderNo?.toLowerCase()?.includes(search?.toLowerCase())
      );
    }
  ...
  const handleDelete = (id, title) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const myurl = `${V_URL}/user/delete-qualified-welder`;
                          ...
                          <div className="doctor-table-blk">
                            ...
                            <div className="doctor-search-blk">
                              ...
                              <div className="add-group">
                                <WeldingAuth>
                                  <Link to="/user/project-store/manage-welder"
                        ...
                        <tbody>
                          {commentsData?.map((elem, i) =>
                            <tr key={elem?._id}>
                              ...
                              <td>{elem?.jointType?.map((e) => e?.jointId?.name)?.join(', ')}</td>
                              ...
                              <td className='status-badge'>
                                {elem.is_epxpired === false ? (
                                  <span className="custom-badge status-green">Active</span>
                                ) : (
                                  <span className="custom-badge status-pink">Expired</span>
                                )}
                              </td>
                              <WeldingAuth>
                                  ...
                                  <div className="dropdown dropdown-action">
                                    ...
                                    <div className="dropdown-menu dropdown-menu-end">
                                      <button type='button' className="dropdown-item"
                                        onClick={() => navigate('/user/project-store/manage-welder', { state: elem })}>
                                        <i className="fa-solid fa-pen-to-square m-r-5"></i>
                                        Edit</button>
                                      <button type='button' className="dropdown-item" onClick={() => handleDelete(elem?._id, elem.welderNo)}><i
