...
const MultiIssueRequest = () => {
    ...
    const handleSubmit = () => {
        ...
        // ADD THE CONDITION PENDING WE NEED TO CHECK THAT AS PER PERTICULAR IMIR PENDING
        // for (const item of updatedData) {
        //     const totalIssueWeight = item.assembly_weight * item.grid_id.grid_qty
        //     const stockItem = stockData?.filter(stock => stock.itemId === item.item_name?._id);
        //     const imirWithSufficientStock = stockItem.find(stock => stock.balance_qty >= totalIssueWeight);
        //     console.log('')
        //     if (!imirWithSufficientStock) {
        //         toast.error(
        //             `Cannot fulfill the requirement of ${totalIssueWeight} for item: ${item.item_name?.name} using a single IMIR. Ensure one IMIR has sufficient stock.`
        //         );
        //         return;
        //     }
        // }
