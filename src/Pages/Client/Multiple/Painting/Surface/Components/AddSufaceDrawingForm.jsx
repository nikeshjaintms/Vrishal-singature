...
const AddSufaceDrawingForm = ({ modalOpen, handleModalClose ,paintNo, handleAddMore, handleCloseModal, editeMode, onSaveItem, updateItem, editItem, modalMode }) => {
    ...
    useEffect(() => {
        ...
        if (modalMode === "edit" && updateItem) {
            setItem({
                ...
                surface_used_grid_qty: updateItem?.surface_used_grid_qty || 0,
        ...
        // When initially entering 'editMode' with an existing editItem
            ...
            setItem({
                ...
                surface_used_grid_qty: editItem?.surface_used_grid_qty || 0,
    ...
    const validationModal = () => {
        ...
        if (!item.surface_used_grid_qty || isNaN(item.surface_used_grid_qty) || parseInt(item.surface_used_grid_qty) <= 0) {
            isValid = false;
            newErrors['surface_used_grid_qty_error'] = "Please enter a valid Qty";
        }
...
//         surface_used_grid_qty: Number(item.surface_used_grid_qty),
...
//     // ✅ Add this line
//     if (paintNo) {
//         bodyFormData.append('paint_system_id', paintNo);
//     }

//     try {
//         await dispatch(manageSurfaceOffer({ bodyFormData })).unwrap();

//         if (paintNo) {
//             await dispatch(getMultiSurface({ paint_system_id: paintNo })).unwrap();
//         }
...
const handleSaveSingleItemToAPI = async () => {
  ...
  try {
    await dispatch(manageSurfaceOffer({ bodyFormData })).unwrap();
    if (paintNo) await dispatch(getMultiSurface({ paint_system_id: paintNo })).unwrap();
    toast.success("Drawing details saved successfully.");
    handleReset();
    handleClose();
  } catch (error) {
                                        ...
                                        <div className="col-md-6">
                                            <div className="input-block local-forms">
                                                ...
                                                <input
                                                    type="number"
                                                    className="form-control"
                                                    name="surface_used_grid_qty"
                                                    value={item.surface_used_grid_qty}
                                                    onChange={handleChange}
                                                />
                                                <div className="error">{err.surface_used_grid_qty_error}</div>
                                            </div>
                                        </div>
                                    </div>
...
export default AddSufaceDrawingForm;