...
const FinalCoatTable = ({
  setSubmitArr,
  paintNo,
  data,
  dispatch_site,
  report_no,
  onAddItem ,is_view = false, isEditMode = false
}) => {
  ...
  const getMultiFinalCoatData = useSelector(
    (state) => state?.getMultiFinalCoat?.user?.data
  );

  useEffect(() => {
    if (getMultiFinalCoatData?.length > 0) {
      setTableData(getMultiFinalCoatData);
      setSubmitArr(getMultiFinalCoatData);
    } else if (paintNo) {
      if (data?._id) {
  ...
  const handleEditFormChange = (e) => {
    ...
    const getFinalCoatOfferTable = () => {
    dispatch(getMioFilterList());
    // dispatch(getMultiFinalCoat());
  };
  ...
  const handleRemoveByDrawing = async (elem) => {
    ...
    try {
      const myurl = `${V_URL}/party/delete-multi-final-coat-offer`;
      ...
      if (data.success === true) {
         ...
         const isManual = !elem.main_id && !elem.drawing_id && !elem.grid_id;
                ...
                if (isManual) {
                  ...
                  // Shw toast and update frontend
                  toast.success("Manual item has been removed!");
                  getFinalCoatOfferTable(); // or remove from local state manually
                  return { skipped: true };
                }
        ...
        const balanceData = [
          {
            ...
            fc_used_grid_qty: elem.fc_used_grid_qty,
        ...
        try {
          const myurl = `${V_URL}/party/mio-grid-balance-update`;
          const response = await axios({
            method: "post",
            url: myurl,
            data: bodyFormData,
                  ...
                  <tbody>
                    {commentsData?.map((elem, i) => (
                      <tr key={i}>
                        ...
                        <td>{elem?.dispatch_report || elem?.dispatch_no || "-"}</td>
                        <td>{elem?.dispatch_site || "-"}</td>
                        <td>{elem?.grid_no || "-"}</td>
                        <td>{elem?.fc_used_grid_qty || "-"}</td>
                        {!data?._id ? (
                          <>
                            {editRowIndex === i ? (
                              <>
                        ...
                        <td>
                          {commentsData
                            .reduce(
                              (total, item) =>
                                total +
                                (parseFloat(item.fc_used_grid_qty) || 0),
                              0
                            )
                            .toFixed(2)}
                        </td>
