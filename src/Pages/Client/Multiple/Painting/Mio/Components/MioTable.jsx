...
const MioTable = ({
  ...
  const getMultiMioData = useSelector(
    (state) => state?.getMultiMio?.user?.data
  );

  useEffect(() => {
    if (getMultiMioData?.length > 0) {
      setTableData(getMultiMioData);
      setSubmitArr(getMultiMioData);
    } else if (paintNo) {
      if (data?._id) {
  ...
  }, [getMultiMioData, paintNo]);

    const getMioOfferTable = () => {
    dispatch(getMultiSurfaceMio());
    dispatch(getMultiMio());
  };
      ...
      if (data.success === true) {
         ...
         const isManual = !elem.main_id && !elem.drawing_id && !elem.grid_id;
                ...
                if (isManual) {
                  ...
                  getMioOfferTable(); // or remove from local state manually
                  return { skipped: true };
                }
        ...
        const balanceData = [
          {
            ...
            mio_used_grid_qty: elem.mio_used_grid_qty,
                        ...
                        <td>
                          {commentsData
                            .reduce(
                              (total, item) =>
                                total +
                                (parseFloat(item.mio_used_grid_qty) || 0),
                              0
                            )
                            .toFixed(2)}
                        </td>
