...
const Transport = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (disable === true) {
        try {
          await dispatch(getAdminTransport({ is_main: false }))
          setDisable(false);
        } catch (error) {
          console.log(error, '!!')
          setDisable(false);
  ...
  const entity = useSelector((state) => state?.getAdminTransport?.user?.data);
  console.log(entity, '@@')
    ...
    if (search) {
      computedComments = computedComments.filter(
        (unit) =>
          ...
          unit.phone?.toString().toLowerCase()?.includes(search?.toLowerCase()) ||
  ...
  const handleDelete = (id, title) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const myurl = `${V_URL}/user/delete-transport`;
            ...
            <div className="row">
              <div className="col-sm-12">
                <ul className="breadcrumb">
                  ...
                  <li className="breadcrumb-item active">Transport List</li>
                </ul>
              </div>
            </div>
          </div>
                      ...
                      <div className="row align-items-center">
                          ...
                          <div className="doctor-table-blk">
                            ...
                            <div className="doctor-search-blk">
                              ...
                              <div className="add-group">
                                <PlanningAuth>
                                  <Link to="/user/project-store/manage-transport"
                        ...
                        <div className="pageDropDown col-auto text-end float-end ms-auto download-grp">
                          <div className="add-group" style={{ marginRight: "10px" }}>
                            <DownloadFormat url={`${V_URL}/user/download-transport-format`} fileName="Transport.xlsx" />
                            <UploadFile url={`${V_URL}/user/upload-transport`} />
                          </div>
                        ...
                        <tbody>
                          {commentsData?.map((elem, i) =>
                            <tr key={elem?._id}>
                              ...
                              <td>{elem?.phone}</td>
                              <td>{elem?.address?.length > 25 ? elem?.address?.slice(0, 25) + '...' : elem?.address || ""}</td>
                                ...
                                <td className="text-end">
                                  <div className="dropdown dropdown-action">
                                    ...
                                    <div className="dropdown-menu dropdown-menu-end">
                                      <button type='button' className="dropdown-item" onClick={() => navigate('/user/project-store/manage-transport', { state: elem })}><i
