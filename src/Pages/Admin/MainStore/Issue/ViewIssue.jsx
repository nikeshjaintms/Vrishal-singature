...
import { getOneIssue } from "../../../../Store/Admin/Transaction/GetOneIssue";

const ViewIssue = () => {
    const getOneIsseItem = useSelector((state => state?.getSingleIss?.user?.data))
    const headers = {
        'Name': 'item_name',
        'Unit': 'unit',
        'M.code': 'm_code',
    ...
    useEffect(() => {
        ...
        if (disable === true) {
            setEntity([]);
            dispatch(getOneIssue({ id: data?._id }));
        }
    }, [navigate, disable]);
    useEffect(() => {
        const fetchData = async () => {
            try {
                await dispatch(getOneIssue({ id: data?._id }));
                setDisable(false)
            } catch (error) {
                setDisable(false)
                console.log(error, '!!')
