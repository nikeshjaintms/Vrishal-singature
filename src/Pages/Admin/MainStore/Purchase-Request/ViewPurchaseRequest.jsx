...
import { getOnePR } from "../../../../Store/Admin/Transaction/GetOnePurchaseRequest";

const ViewPurchaseRequest = () => {
    const getSinglePurchaseRrequestData = useSelector((state => state?.getOnePR?.user?.data))
    ...
    useEffect(() => {
        ...
        if (disable === true) {
            setEntity([]);
            getOnePR({ id: data?._id });
        }
    }, [navigate, disable, getSinglePurchaseRrequestData]);

    useEffect(() => {
        const fetchData = async () => {
            try {
                await dispatch(getOnePR({ id: data?._id }))
                setDisable(false)
            } catch (error) {
                console.log(error, '!!')
                setDisable(false)
    ...
    const handleBack = () => {
        navigate('/admin/purchase-request')
    }
    return (
                        ...
                        <div className="row">
                            <div className="col-sm-12">
                                <ul className="breadcrumb">
                                    ...
                                    <li className="breadcrumb-item">
                                        <Link to="/admin/purchase-request">View Purchase Request</Link>
                                    </li>
                                    <li className="breadcrumb-item">
                                        <i className="feather-chevron-right"></i>
                                    </li>
                                    <li className="breadcrumb-item active">Purchase Request Items</li>
                                </ul>
                            </div>
                        </div>
                    </div>
