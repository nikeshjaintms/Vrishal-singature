...
import { getAdminPMSStock } from "../../../../Store/Admin/PMS/GetPMSStock";
...
const PMSStock = () => {
    ...
    const fetchData = () => {
        dispatch(getAdminPMSStock());
        setDisable(false);
    }

    const entity = useSelector((state) => state.getAdminPMSStock?.user?.data);
    ...
    const handleDownload = (options) => {
        if (options?.iselxs) {
            ...
            PdfDownloadErp({ apiMethod: 'post', url: 'stock-list-xlsx', body: bodyFormData, is_admin: true });
        } else {
            const bodyFormData = new URLSearchParams();
            bodyFormData.append('print_date', true);
            PdfDownloadErp({ apiMethod: 'post', url: `download-stock-list`, body: bodyFormData, is_admin: true });
        }
    }
