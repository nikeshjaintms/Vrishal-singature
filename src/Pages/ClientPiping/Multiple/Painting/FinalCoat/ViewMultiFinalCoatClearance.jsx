...
const ViewMultiFinalCoatClearance = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const data = location.state; // contains Final Coat Inspection report object

  console.log('Final Coat Inspection Data:', data);
  ...
  const fetchPdf = async () => {
    try {
      ...
      const res = await axios.post(
        `${V_URL}/party/download-piping-final-coat-client`,
        {
  ...
  const submitFinalCoatStatus = async (statusType) => {
    ...
    try {
      ...
      const res = await axios.post(`${V_URL}/party/update-piping-final-coat-status`, payload, {
        headers: {
          Authorization: 'Bearer ' + localStorage.getItem('PARTY_TOKEN'),
        },
      });
      ...
      if (res.data.success) {
        toast.success('Final Coat Clearance updated successfully');
        setShowRandomItems(false);
        fetchPdf();
        navigate("/party/piping-store/final-coat-clearance-management");
      } else {
        ...
        <div className="content">
          ...
          <div className="page-header">
            <ul className="breadcrumb">
              ...
              <li className="breadcrumb-item">
                <Link to="/party/piping-store/final-coat-clearance-management">Final Coat Clearance</Link>
              </li>
              <li className="breadcrumb-item"><i className="feather-chevron-right"></i></li>
              <li className="breadcrumb-item active">View Final Coat Clearance</li>
            </ul>
          </div>
          ...
          <div className="card">
            <div className="card-body">
              <h4 className="mb-3">Final / Top Coat Details</h4>
              <div className="row">
            ...
            <div className="card-body">
              ...
              {showButtons && (
                <>
                  ...
                  <div className="mt-3">
                    <button className="btn btn-primary me-2" onClick={() => submitFinalCoatStatus('REVIEWED')}>
                      REVIEWED
                    </button>

                    <button className="btn btn-warning me-2" onClick={() => submitFinalCoatStatus('WITNESSED')}>
                      WITNESSED
                    </button>
              ...
              {pdfUrl && (
                <div className="mt-4">
                  <iframe
                    src={pdfUrl}
                    title="Final Coat Inspection PDF"
                    width="100%"
                    height="700px"
              ...
              {showRandomItems && randomItems.length > 0 && (
                <div className="mt-3">
                      ...
                      <tbody>
                        {randomItems.map((item, index) => (
                          <tr key={item._id}>
                            ...
                            <td>{item.rev}</td>
                            <td>{item.spool_no}</td>
                            <td>{item.average_dft_final_coat}</td>
                            <td>
                              <input
                                type="text"
                  ...
                  <button className="btn btn-success mt-2" onClick={() => submitFinalCoatStatus('RANDOM WITNESSED')}>
...
export default ViewMultiFinalCoatClearance;
