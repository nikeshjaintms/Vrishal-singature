...
const ViewMultiClearFT = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const data = location.state; // contains FT report object
  ...
  const prepareRandomWitnessedItems = () => {
    ...
    const mappedItems = items.map((item) => ({
      ...
      status: item?.is_accepted === true || item?.is_accepted_qc === true ? 'Accepted' : 'Rejected',
      selected: false,
    }));
  ...
  /* ================= UPDATE STATUS ================= */
  const submitFTStatus = async (statusType) => {
    if (!clientDate) {
    ...
    try {
      ...
      const res = await axios.post(`${V_URL}/party/update-piping-ft-status`, payload, {
        headers: {
          Authorization: 'Bearer ' + localStorage.getItem('PARTY_TOKEN'),
        },
      });
      ...
      if (res.data.success) {
        toast.success('FT Clearance updated successfully');
        setShowRandomItems(false);
        fetchPdf();
        navigate("/party/piping-store/ft-clearance-management");
      } else {
        ...
        <div className="content">
          ...
          <div className="page-header">
            <ul className="breadcrumb">
              ...
              <li className="breadcrumb-item">
                <Link to="/party/piping-store/ft-clearance-management">FT Acc / Rej</Link>
              </li>
              <li className="breadcrumb-item"><i className="feather-chevron-right"></i></li>
              <li className="breadcrumb-item active">View FT Clearance</li>
            </ul>
          </div>
          ...
          <div className="card">
            <div className="card-body">
              <h4 className="mb-3">FT Details</h4>
              <div className="row">
            ...
            <div className="card-body">
              ...
              {showButtons && (
                <>
                  ...
                  <div className="mt-3">
                    <button className="btn btn-primary me-2" onClick={() => submitFTStatus('REVIEWED')}>
                      REVIEWED
                    </button>

                    <button className="btn btn-warning me-2" onClick={() => submitFTStatus('WITNESSED')}>
                      WITNESSED
                    </button>
              ...
              {showRandomItems && randomItems.length > 0 && (
                <div className="mt-3">
                      ...
                      <tbody>
                        {randomItems.map((item, index) => (
                          <tr key={item._id || index}>
                            ...
                            <td>{item.observation_12}</td>
                            <td>{item.observation_6}</td>
                            <td>{item.status}</td>
                          </tr>
                        ))}
                      </tbody>
                  ...
                  <button className="btn btn-success mt-2" onClick={() => submitFTStatus('RANDOM WITNESSED')}>
...
export default ViewMultiClearFT;
