...
export const getProcurementRequestById = createAsyncThunk(
  "procurementRequest/getById",
  async (payload, { rejectWithValue }) => {
    try {
      const id = payload.id;
      const response = await axios.post(
        `${V_URL}/user/pr/get-procurement-request-by-id`,
        { id },
        { headers: { Authorization: "Bearer " + localStorage.getItem("PAY_USER_TOKEN") } }
      );
...
export default procurementRequestSlice.reducer;
