...
export const getStockSurfaseDataInMio = createAsyncThunk(
  "/user/piping-get-stock-surface-data-in-mio",
  async (args = {}) => {
    try {
      const { page, limit, search, DATA, isMIO } = args;
      const myurl = `${V_URL}/user/piping-get-stock-surface-data-in-mio`;
...
const getStockSurfaseDataInMioSlice = createSlice({
  name: "getStockSurfaseDataInMio",
  initialState: {
  ...
  extraReducers: (builder) => {
    builder
      .addCase(getStockSurfaseDataInMio.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(getStockSurfaseDataInMio.fulfilled, (state, action) => {
        state.user = action.payload; // full response
        state.loading = false;
        state.error = null;
      })
      .addCase(getStockSurfaseDataInMio.rejected, (state, action) => {
...
export default getStockSurfaseDataInMioSlice.reducer;
