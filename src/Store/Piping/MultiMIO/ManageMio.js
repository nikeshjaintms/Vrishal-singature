...
export const manageMioOfferTable = createAsyncThunk(
  "/user/piping-manage-multi-mio-offer",
  async ({ bodyFormData }) => {
    try {
      const myurl = `${V_URL}/user/piping-manage-multi-mio-offer`;
      const response = await axios({
        method: "post",
        url: myurl,
        data: bodyFormData,
...
const manageMioOfferSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(manageMioOfferTable.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(manageMioOfferTable.fulfilled, (state, action) => {
        state.user = action.payload;
        state.loading = false;
        state.error = null;
      })
      .addCase(manageMioOfferTable.rejected, (state, action) => {
        state.user = null;
        state.loading = false;
        state.error = action.error.message;
      });
