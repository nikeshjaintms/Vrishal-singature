...
export const getPipingMultiSurfaceOffer = createAsyncThunk(
  "/user/piping-get-multi-surface",
  async (args = {}) => {
    try {
...
const getPipingMultiSurfaceOfferSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(getPipingMultiSurfaceOffer.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(getPipingMultiSurfaceOffer.fulfilled, (state, action) => {
        state.user = action.payload; // full response
        state.loading = false;
        state.error = null;
      })
      .addCase(getPipingMultiSurfaceOffer.rejected, (state, action) => {
...
export default getPipingMultiSurfaceOfferSlice.reducer;
