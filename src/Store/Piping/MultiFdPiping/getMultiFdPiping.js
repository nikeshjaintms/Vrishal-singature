...
export const getMultiFdPiping = createAsyncThunk(
  "/party/getMultiFdPiping",
...
const getMultiFdPipingSlice = createSlice({
    name: "getMultiFdPiping",
    initialState: {
       pendingList: null,
    completedList: null,
        user: null,
    ...
    extraReducers: (builder) => {
        builder
            .addCase(getMultiFdPiping.pending, (state) => {
                state.loading = true;
                state.error = null;
            })
            .addCase(getMultiFdPiping.fulfilled, (state, action) => {
 ...
 if (String(status) === "1") {
    ...
    state.completedList = action.payload;
            ...
            .addCase(getMultiFdPiping.rejected, (state, action) => {
                state.user = null;
                state.loading = false;
...
export default getMultiFdPipingSlice.reducer;