...
export const getUserPwhtMaster = createAsyncThunk(
  '/issue/getUserPwhtMaster', // The action type string
  async (
    ...
    try {
      ...
      const response = await axios.post(
        `${V_URL}/user/get-all-pwht`, // Your backend URL
...
const getUserPwhtMasterSlice = createSlice({
  name: "getUserPwhtMaster",
  initialState: {
  ...
  extraReducers: (builder) => {
    builder
      .addCase(getUserPwhtMaster.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(getUserPwhtMaster.fulfilled, (state, action) => {
      ...
      .addCase(getUserPwhtMaster.rejected, (state, action) => {
...
export default getUserPwhtMasterSlice.reducer;
