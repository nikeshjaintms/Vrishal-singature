...
export const getUserPickling = createAsyncThunk(
  "/issue/getUserPickling",
  async ({ page = 1, limit = 10, search = "" } = {}, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        `${V_URL}/user/piping-list-multi-pickling-offer`,
...
const getUserPicklingSlice = createSlice({
  name: "getUserPickling",
  initialState: {
    user: null,
    loading: false,
    error: null,
  ...
  extraReducers: (builder) => {
    builder
      .addCase(getUserPickling.pending, (state) => {
        console.log("Fetching PMI Offer: pending");
        state.loading = true;
        state.error = null;
      })
      .addCase(getUserPickling.fulfilled, (state, action) => {
      ...
      .addCase(getUserPickling.rejected, (state, action) => {
...
export default getUserPicklingSlice.reducer;
