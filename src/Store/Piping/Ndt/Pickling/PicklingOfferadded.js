...
export const getUserPicklingAdded = createAsyncThunk(
    "/issue/getUserPicklingAdded",
    async ({ page = 1, limit = 10, search = "" } = {}, { rejectWithValue }) => {
        try {
            ...
            const response = await axios.post(
                `${V_URL}/user/piping-list-multi-pickling-inspection`,
...
const getUserPicklingAddedSlice = createSlice({
    name: "getUserPicklingAdded",
    initialState: {
        user: null,
        loading: false,
        error: null,
    ...
    extraReducers: (builder) => {
        builder
            .addCase(getUserPicklingAdded.pending, (state) => {
                console.log("Fetching FinalCoatShades: pending");
                state.loading = true;
                state.error = null;
            })
            .addCase(getUserPicklingAdded.fulfilled, (state, action) => {
            ...
            .addCase(getUserPicklingAdded.rejected, (state, action) => {
...
export default getUserPicklingAddedSlice.reducer;