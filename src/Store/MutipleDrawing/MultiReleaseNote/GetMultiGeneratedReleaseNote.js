...
export const GetMultiGenReleaseNote = createAsyncThunk(
  ...
  async ({ page , limit, search } = {}) => {
    try {
      const baseUrl = `${V_URL}/user/list-multi-release-generate`;
      const Project = localStorage.getItem("U_PROJECT_ID");
...
const GetMultiGenReleaseNoteSlice = createSlice({
  name: "GetMultiGenReleaseNote",
  initialState: {
    user: null,
    loading: false,
    error: null,
  ...
  extraReducers: (builder) => {
    builder
      .addCase(GetMultiGenReleaseNote.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(GetMultiGenReleaseNote.fulfilled, (state, action) => {
        state.user = action.payload;
        state.loading = false;
        state.error = null;
      })
      .addCase(GetMultiGenReleaseNote.rejected, (state, action) => {
        state.user = null;
        state.loading = false;
...
export default GetMultiGenReleaseNoteSlice.reducer;
